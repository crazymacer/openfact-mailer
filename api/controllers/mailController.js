...
const customEmail = require('../helpers/customEmail');
...
const isOfflineRaw = process.env.OFFLINE || 'true';
const isOffline = (isOfflineRaw === 'true');

exports.sendMailCustom = async (req, res) => {
  ...
  try {
    ...
    if (!isOffline) {
      ...
      const email = customEmail.getData(req);

      // Validate email parameters
      if (validationHelper.validateCustomEmail(email)) {

        // TODO: Validar cuando vienen varios correos separados por ";"
        const isBlacklisted = await blacklistService.isBlacklisted(email.data.recipients);

        // Validate blacklist
        if (isBlacklisted) {
          resMessage = `El correo de destino '${email.data.recipients}' se encuentra en la lista negra.`;
          resSuccess = false;
        } else {
          ...
          resSuccess = true;
          resMessage = `Mensaje enviado correctamente`;
        }

      } else {
        resMessage = 'Debe validar los parámetros del cuerpo y configuración';
        resSuccess = false;
      }
      ...
      return res.json({ success: true, message: "En mantenimiento" }); // TODO: En mantenimiento 30/01/2021  
    }
...
exports.sendMail = async (req, res) => {
  ...
  try {
    ...
    if (!isOffline) {
      ...
      const isBlacklistedRuc = await blacklistService.isBlacklistedRuc(senderRuc);
      ...
      if (!isBlacklistedRuc) {
        ...
        if (cfgEmailHost != null
          ...
          if (clientEmail != null && clientEmail != ""
            ...
            if (mailSecret == cfgsecretWord) {
              ...
              const isBlacklisted = await blacklistService.isBlacklisted(clientEmail);
              ...
              if (!isBlacklisted) {
                ...
                res.json({ success: false, message: `El correo de destino '${clientEmail}' se encuentra en la lista negra.` });
              }
        ...
        res.json({ success: false, message: "El RUC se encuentra en la lista negra" });
      }
    } else {
      return res.json({ success: true, message: "En mantenimiento" }); // TODO: En mantenimiento 30/01/2021  
    }
